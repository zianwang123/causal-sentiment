...
const sections = [
  ...
  },
  {
    ...
    content:
      "Click \"Scenario Engine\" (top-right) to open the scenario panel. Click \"Generate Scenario\" \u2014 a 4-agent pipeline (Researcher \u2192 Historian \u2192 Strategist \u2192 Mapper) scans news across 12 macro domains, finds historical parallels, and generates 2\u20133 branching scenarios with probability weights, causal chains, and calibrated shocks. Each branch has a narrative, temporal layers (hours \u2192 days \u2192 weeks \u2192 months), mapped shocks, and suggested new nodes/edges. Click a branch to expand. Edit shock values before applying. Click \"Apply to Graph\" to see the cascade with non-linear shock interaction. Click \"Compare\" to select 2 branches side-by-side with color-coded node impacts. Click \"Chain\" on any branch to generate a follow-up scenario from that branch\u2019s outcome. Quick triggers show 5 diverse events across geopolitics, monetary policy, trade, tech, energy, and more. Export as JSON.",
  },
  {
    id: "features",
    title: "More Features",
    items: [
      ...
      { label: "RSS News", desc: "30 curated financial RSS feeds (no API key needed) with source reliability tiers" },
      { label: "Trending Detection", desc: "Auto-triggers agent when 3+ sources converge on the same topic" },
      { label: "Agent Log", desc: "Inspect past analysis runs with full tool call inputs and outputs (click to expand)" },
